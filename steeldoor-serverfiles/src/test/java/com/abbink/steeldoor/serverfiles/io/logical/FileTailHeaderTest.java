...
public class FileTailHeaderTest {
	
	@Parameters
	public static Collection<Object[]> data() {
		byte typeId = File.TYPE_ID;
		long fileId = (1L<<56)+(2L<<48)+(3L<<40)+(4L<<32)+(5L<<24)+(6L<<16)+(7L<<8)+8L;
		long headId = (10L<<56)+(20L<<48)+(30L<<40)+(40L<<32)+(50L<<24)+(60L<<16)+(70L<<8)+80L;
		...
		return Arrays.asList(new Object[][] {
				{typeId, fileId, headId, ownerId, cookie, false, length, tailId, new byte[] {1, 1,2,3,4,5,6,7,8, 10,20,30,40,50,60,70,80, 4,3,2,1, 101,102,103,104,105,106,107,108, 0, 8,7,6,5,4,3,2,1, 0,0,0,0,0,0,0,0}},
				{typeId, fileId, headId, ownerId, cookie, true,  length, tailId, new byte[] {1, 1,2,3,4,5,6,7,8, 10,20,30,40,50,60,70,80, 4,3,2,1, 101,102,103,104,105,106,107,108, 1, 8,7,6,5,4,3,2,1, 0,0,0,0,0,0,0,0}}
		});
	}
	
	private byte typeIdInput;
	private long fileIdInput;
	private long headIdInput;
	...
	public FileTailHeaderTest(byte typeIdInput, long fileIdInput, long headIdInput, int ownerIdInput, long cookieInput, boolean existsInput, long lengthInput, long tailIdInput, byte[] expectedHeader) {
		this.typeIdInput = typeIdInput;
		this.fileIdInput = fileIdInput;
		this.headIdInput = headIdInput;
	...
	public void test() throws IOException {
		byte[] actual = TestableFileTailWriter.generateHeader(typeIdInput, fileIdInput, headIdInput, ownerIdInput, cookieInput, existsInput, lengthInput, tailIdInput);
